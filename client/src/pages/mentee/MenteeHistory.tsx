...
import { Expired } from "../../interfaces/mentee.interface";
...
const MenteeHistory = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const response = await axiosPrivate.get("/mentee/expired", {
        ...
        try {
          const fetchedMentees = await Promise.all(
            expired.map(async (menteeObj) => {
              const imageId = menteeObj?.mentorDetails?.profile_img;
              if (imageId) {
          ...
          <>
            <div className="px-2 md:px-10 md:py-10 w-full">
              <div className="py-5 font-bold text-2xl">
                <h1 className="text-white">My History</h1>
              </div>
              <hr />
                        ...
                        <div className="flex">
                          ...
                          <div className="px-2 py-2 font-bold">
                            <h1 className="text-xl text-white">
                              {plan?.mentorDetails?.first_name}
                              {plan?.mentorDetails?.last_name}
                            </h1>
                            <h1 className="mt-2 uppercase text-sm text-white">
                              {plan?.mentorDetails?.job_title}
                            </h1>
                          </div>
                        </div>
                          ...
                          <figcaption>
                            <div className="flex justify-around">
                              <button
                                ...
                                onClick={() => {
                                  navigate(
                                    `/my-mentors/paymentDetails/${plan?._id}`
                                  );
                                }}
                              >
